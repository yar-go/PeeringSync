...
from .statistic import Information
...
class LoadManager:
    ...
    def __init__(self, key: 'Key', seed: 'Seed', destination: str, peer_id: bytes, max_connections: int = 10,
                 log_func: typing.Callable[[str], None] = None, peer_connect_timeout=10, piece_receive_timeout=5):
        self._key = key
        self._seed = seed
        self._peer_id = peer_id
        self._piece_len = seed.piece_length
        self._pieces = seed.pieces
        self._length = seed.length
        ...
        self._seed_updater_task: asyncio.Task | None = None
        self._map_updater_task: asyncio.Task | None = None

        self.filesmanager = FilesManager.open(destination, seed.files, self._piece_len, seed.pieces, seed.version)
        self._peers: [Peer] = list()
        self._outter_peers: [Peer] = list()

        self._connected_peers: list[Peer] = list()
        self._interesting_peers: list[Peer] = list()
        self._listhening_tasks: dict[Peer, asyncio.Task] = dict()

        self._queue_pieces = asyncio.PriorityQueue(len(self.filesmanager.bitfield))
        self._requested_task_per_peer: dict[Peer, asyncio.Task] = dict()
        self._requested_peers_per_num: dict[int, list[Peer]] = dict()
        ...
        self._down_state = self.filesmanager.bitfield.count_available_blocks() * self._piece_len

        self._answering_task = None
        self._requesting_task = None
    ...
    async def run(self):
        ...
        self._answering_task = asyncio.Task(self.start_answering())
        if not self.filesmanager.bitfield.full():
            self._requesting_task = asyncio.Task(self.start_download())
        self._interesting_supporter = asyncio.Task(self._support_interesting())
        self._seed_updater_task = asyncio.Task(self._seed_updater())
        self._map_updater_task = asyncio.Task(self._peers_map_updater())

        await self._shutdown_future

    def reg_updated_peers_clb(self, clb):
        self._updated_peers_clb = clb
    ...
    async def _peers_map_updater(self):
        while True:  # norma
            new_items = list()
            for p in self._connected_peers:
                item_key = BenCoder.encode([p.ip, p.port, p.id])
                other_map = await p.request_peers()
                self._peer_map[item_key] = other_map
                new_items.append(item_key)
            for k in list(self._peer_map.keys()):
                if k not in new_items:
                    self._peer_map.pop(k)
            await asyncio.sleep(1)

    async def update_seed(self, seed: 'Seed'):
        if not self._key.verify(seed.raw, seed.signature):
            return False

        if self._requesting_task:
            self._requesting_task.cancel()
            self._interesting_supporter.cancel()
            try: await self._requesting_task
            except asyncio.CancelledError: pass
            try: await self._interesting_supporter
            except asyncio.CancelledError: pass
        ...
        self.filesmanager.relocate(seed)

        for p in self._connected_peers:
            await p.send_bitfield(self.filesmanager.bitfield)

        print(self.filesmanager.bitfield.bits)
        if not self.filesmanager.bitfield.full():
            self._requesting_task = asyncio.Task(self.start_download())
        ...
        if self._updated_peers_clb:
            self._updated_peers_clb(seed)
        return True

    async def _seed_updater(self):
        while self._upload_work or self._download_work:
            versions = list()
            for p in self._connected_peers:
                if p.bitfield:
                    versions.append((p.bitfield.version, p,))
            versions.sort(reverse=True, key=lambda a: a[0])
            if versions and versions[0][0] > self._seed.version:
                newer_seed = await versions[0][1].request_seed()
                await self.update_seed(newer_seed)
            await asyncio.sleep(1)
        ...
        while self._download_work:
            ...
            if not next_block_index:
                if self._queue_pieces.empty():
                    ...
                    continue
                _, next_block_index = await self._queue_pieces.get()
    ...
    async def start_answering(self) -> None:
        """Розпочати відвантаження контенту торенту іншим підключеним пірам"""
        self._log_func("Start answering")
        while self._upload_work:
            # if self.upload_queue.empty():
            #     await asyncio.sleep(0)   # TODO make reduce huy
            #     continue
            try:
                peer, type_, index, begin, length = await self.upload_queue.get()
            ...
            if type_ == "seed":
                await peer.send_seed(self._seed)
            if type_ == "peers":
                item_key = BenCoder.encode([peer.ip, peer.port, peer.id])
                to_send = {p_key:self._peer_map[p_key] for p_key in self._peer_map if p_key != item_key}
                await peer.send_peers(to_send)

        self._log_func("Upload is stopped")

    async def _request_piece(self, peer: 'Peer', piece_index: int):
        ...
        print("SIGMA REQUEST", piece_index)
        block_size = 2 ** 16
        ...
        if self.filesmanager.bitfield.has(piece_index):
            self._requested_peers_per_num.pop(piece_index, None)
            return False
    ...
    async def _support_interesting(self) -> None:
        ...
        while True:
            for peer in self._connected_peers:
                if (peer.bitfield
                        and peer.bitfield.version == self._seed.version
                        and self.filesmanager.bitfield.count_missing_blocks(peer._bitfield)
                        and peer.bitfield.version == self._seed.version):
                    ...
                    if peer.am_interesting:
                        ...
                        if peer in self._interesting_peers:
    ...
    def _interesting_iter(self) -> typing.Iterable[typing.Union['Peer', None]]:
    ...
    async def _prioritize(self) -> None:
        ...
        [await self._queue_pieces.put(tuple(b)) for b in counter
    ...
    async def _support_connected_peers(self, timeout_to_connect: int = 10, once_to_connect=10) -> None:
        ''' Лише створюються вихідні з'єднання '''
        print( "SIGMAAAAAA\n\n\n\n\n")
        while True:
            ...
            # Обробка вхідних підключень
            if self._outter_peers:
                outter_peer = self._outter_peers.pop()
                if outter_peer:
                    p = await self._connect_peer(outter_peer, outter=True)
                    self._peers.append(p)

            await asyncio.sleep(0.1)

    def add_unconnected_peers(self, peers: typing.Sequence['Peer']):
        new_peers = list(filter(lambda a: a not in self._peers, peers))
        self._peers.extend(new_peers)

    def add_connected_peer(self, peer):
        self._outter_peers.append(peer)

    async def _connect_peer(self, peer: 'Peer', timeout: int = 10, outter=False):
        if not outter:
            k = await peer.connect(self._key, self._peer_id, timeout=timeout)
        else: k = False
        if k or (not k and outter):
    ...
    # def _upload_request(self, peer: 'Peer', type:str, index: int, begin: int, lenght: int):
    #     if self.upload_queue.full(): return
    #     self.upload_queue.put_nowait((peer, index, begin, lenght,))
    def _upload_request(self, peer: 'Peer', type: str, index: int, begin: int, lenght: int):
        if self.upload_queue.full(): return
        self.upload_queue.put_nowait((peer, type, index, begin, lenght,))

    def _send_haves(self, index: int) -> None:
        for peer in self._connected_peers:
            if peer.bitfield.version == self._seed.version:
    ...
    async def shutdown(self) -> None:
        ...
        if self._seed_updater_task:
            self._seed_updater_task.cancel()
            try:
                await self._seed_updater_task
            except asyncio.CancelledError:
                pass
            self._seed_updater_task = None

        if self._map_updater_task:
            self._map_updater_task.cancel()
            try:
                await self._map_updater_task
            except asyncio.CancelledError:
                pass
            self._map_updater_task = None
    ...
    def get_stat(self) -> Information:
        ...
        return Information(total_size=length, uploaded_per_session=uploaded, downstate=self._down_state,
                           downloaded_per_session=downloaded, left=left, peers_count=peers_count,
                           connected_peers=connected, interesting_peers=interesting,
                           file_path=self.filesmanager.destination, identification=self._key.network_identifier_str,
                           seed_ver=self._seed.version, is_download=self._download_work)
